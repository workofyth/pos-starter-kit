...
export async function POST(req: Request) {
  try {
    ...
    const requestBody = {
      ...
      phone: "081234567890", // Better-auth might not have phone, keeping a default if missing
    ...
    console.log("iPaymu Request Body Sent:", jsonBody);
    ...
    console.log("iPaymu Headers:", { va, signature, timestamp });
    ...
    console.log("iPaymu Response Data:", JSON.stringify(data, null, 2));

    if (data.Status === 200 && data.Success) {
      return NextResponse.json({
        success: true,
        qrString: data.Data.QrString,
        qrImage: data.Data.QrImage || data.Data.QrTemplate,
        sessionId: data.Data.SessionID,
        transactionId: data.Data.TransactionId,
      });
    } else {
      console.error("iPaymu Error:", data);
      return NextResponse.json({ success: false, error: data.Message || "Failed to generate QRIS" }, { status: 400 });
    }
